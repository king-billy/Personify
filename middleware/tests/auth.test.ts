...
describe("Auth Routes", () => {
	it("GET /spotify-auth/login should redirect to Spotify", async () => {
		const res = await request(app).get("/spotify-auth/login");

		expect(res.statusCode).toBe(302);
		expect(res.header.location).toContain("https://accounts.spotify.com");
	});

	it("GET /spotify-auth/login should return 500 if SPOTIFY_CLIENT_ID is missing", async () => {
	...
	it(`GET /spotify-auth/callback without any query should return 400 or error`, async () => {
		const res = await request(app).get("/spotify-auth/callback");

		expect(res.statusCode).toBeGreaterThanOrEqual(400); // returns 400
		expect(res.text.toLowerCase()).toContain("missing"); // or "error"
	});

	it(`POST /spotify-auth/refresh with missing body should return 400 with matched error message — missing "refresh_token"`, async () => {
		const res = await request(app).post("/spotify-auth/refresh").send({}); // No refresh_token

		expect(res.statusCode).toBe(400);
		expect(res.text).toMatch(/missing authorization refresh_token/i);
	});

	it(`POST /spotify-auth/refresh with mocked data should return 500 with matched error message — invalid "refresh_token`, async () => {
		...
		const res = await request(app).post("/spotify-auth/refresh").send({
